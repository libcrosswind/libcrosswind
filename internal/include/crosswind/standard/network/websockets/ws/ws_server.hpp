        ...
        namespace websockets{
            ...
            public:
                ...
                struct Callbacks {
                    ...
                    std::function<void(std::shared_ptr<Connection>, const asio::error_code&)> onerror;
                ...
                void send_close(std::shared_ptr<Connection> connection, int status, const std::string& reason="") {
                    ...
                    send(connection, response, [](const asio::error_code& ec){}, 136);
                }
            ...
            protected:
                ...
                SocketServerBase(unsigned short port, size_t num_threads, size_t timeout_request, size_t timeout_idle) :
                        asio_endpoint(asio::ip::tcp::v4(), port), asio_acceptor(asio_io_service, asio_endpoint), num_threads(num_threads),
                        timeout_request(timeout_request), timeout_idle(timeout_idle) {}

                virtual void accept()=0;

                std::shared_ptr<asio::deadline_timer> set_timeout_on_connection(std::shared_ptr<Connection> connection, size_t seconds) {
                    std::shared_ptr<asio::deadline_timer> timer(new asio::deadline_timer(asio_io_service));
                ...
                void write_handshake(std::shared_ptr<Connection> connection, std::shared_ptr<asio::streambuf> read_buffer) {
                ...
                void read_message(std::shared_ptr<Connection> connection,
                        std::shared_ptr<asio::streambuf> read_buffer, Callbacks& callbacks) {
                ...
                void connection_error(std::shared_ptr<Connection> connection, const Callbacks& callbacks, const asio::error_code& ec) {
                    timer_idle_cancel(connection);
                    connections_mutex.lock();
                    connections.erase(connection);
                    connections_mutex.unlock();
                    if(callbacks.onerror) {
                        asio::error_code ec_tmp=ec;
                ...
                void timer_idle_init(std::shared_ptr<Connection> connection) {
                    if(timeout_idle>0) {
                        connection->timer_idle=std::unique_ptr<asio::deadline_timer>(new asio::deadline_timer(asio_io_service));
                        connection->timer_idle->expires_from_now(std::chrono::seconds(timeout_idle));
                        timer_idle_expired_function(connection);
                    }
                }
                void timer_idle_reset(std::shared_ptr<Connection> connection) {
                   if(timeout_idle>0 && connection->timer_idle->expires_from_now(std::chrono::seconds(timeout_idle))>0) {
                        timer_idle_expired_function(connection);
                    }
                }
            ...
            private:
                void accept() {
                    ...
                    asio_acceptor.async_accept(*connection->socket, [this, connection](const asio::error_code& ec) {
