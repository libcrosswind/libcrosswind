...
#include <crosswind/database/sqlite/sqlite_error.hpp>
...
class cw::database::sqlite::sqlite_transaction_failed: public sqlite_error {
public:
    sqlite_transaction_failed(const int status): error(status) {}
};


class cw::database::sqlite::sqlite_result{
public:
...
private:
  bool step_result(const std::shared_ptr<sqlite3_stmt> &stmt) {
    ...
    case SQLITE_BUSY:   throw sqlite_transaction_failed(status);
    default:            throw sqlite_error(stmt);
    }
  }
