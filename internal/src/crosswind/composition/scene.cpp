...
auto cw::composition::scene::get_camera(const std::string& camera_name){
...
auto& cw::composition::scene::get_group_map(){
	return group_map;
}

auto& cw::composition::scene::get_actor_map(){
	return actor_map;
}
...
void cw::composition::scene::post_event(const std::function<void()>& event, const bool& continuous = false){
